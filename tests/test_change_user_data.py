...
def test_change_user_data_authorized(create_user_and_get_token, user_methods, field_to_change, new_value):
    user_data, access_token = create_user_and_get_token
    update_data = {field_to_change: new_value}  # Подготавливаем данные для замены
    change_response = user_methods.change_user_data(access_token, update_data)
...
def test_change_user_data_unauthorized(user_methods):
    user_data = generate_user_body()
    response = user_methods.change_user_data_unauthorized(user_data)
